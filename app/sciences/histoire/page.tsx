...
/* ── Palette unique (indigo/violet, cohérent avec ScienceLayout) ── */
const ACCENT = '#818CF8';
        ...
        return (
          <g key={era.label}>
            <rect x={x1} y={yBar} width={bw} height={h} rx="2"
              fill={`${ACCENT}20`} stroke={ACCENT} strokeWidth="1" strokeOpacity="0.6" />
            {bw > 20 && (
              <text x={(x1 + x2) / 2} y={yBar + h / 2 + 0.5} textAnchor="middle"
                fontSize="5.5" fill={ACCENT_SOFT} fontFamily="'Exo 2',sans-serif" fontWeight="700"
        ...
        return (
          <g key={e.label}>
            ...
            <line x1={x} y1={e.yLabel + 6} x2={x} y2={yBar}
              stroke={ACCENT_SOFT} strokeWidth="0.8" opacity="0.55" strokeDasharray="2 1.5" />
            {/* Dot */}
            <circle cx={x} cy={e.yLabel + 6} r="2.5" fill={ACCENT_SOFT} opacity="0.85" />
            {/* Label */}
            <text x={x} y={e.yLabel} textAnchor="middle" fontSize="5" fill={ACCENT_SOFT}
...
function FigurePill({ name }: { name: string }) {
  ...
  const style = { background: `${ACCENT}15`, color: ACCENT_SOFT };
  if (id) {
    return (
      <Link
        ...
        style={{ ...style, background: `${ACCENT}22` }}
      >
        {name}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="space-y-5">
            {REVOLUTIONS.map((rev) => (
              <div key={rev.n} className="rounded-2xl border border-white/8 p-6"
                style={{ background: 'rgba(255,255,255,0.02)', borderTopColor: `${ACCENT}50`, borderTopWidth: 2 }}>
                <div className="flex flex-wrap items-start gap-4 mb-4">
                  <span className="w-9 h-9 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0"
                    style={{ background: `${ACCENT}20`, color: ACCENT_SOFT, fontFamily: "'Exo 2', sans-serif" }}>
                    {rev.n}
                  </span>
                    ...
                    <ul className="space-y-1">
                      {rev.discoveries.map(d => (
                        <li key={d} className="flex items-start gap-1.5 text-xs text-gray-400">
                          <span style={{ color: ACCENT_SOFT }} className="flex-shrink-0">·</span>{d}
                        </li>
                      ))}
                    </ul>
                  </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {KEY_MOMENTS.map((m) => (
              <div key={m.year} className="rounded-xl border border-white/8 p-4 hover:scale-[1.01] transition-all duration-200"
                style={{ background: 'rgba(255,255,255,0.02)', borderTopColor: `${ACCENT}50`, borderTopWidth: 2 }}>
                <div className="text-sm font-bold font-mono mb-2" style={{ color: ACCENT_SOFT }}>{m.year}</div>
        ...
        <div className="pt-10 border-t border-white/6">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {[
              { href: '/sciences/timeline',   label: 'Timeline',       desc: 'Frise chronologique interactive' },
              { href: '/sciences/scientists', label: 'Scientifiques',  desc: 'Portraits des grandes figures' },
              { href: '/sciences/formules',   label: 'Formules',       desc: 'Équations fondamentales' },
            ].map(l => (
              <Link key={l.href} href={l.href} className="group rounded-xl border border-white/8 p-4 hover:border-white/20 transition-all duration-200 block"
                ...
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-sm" style={{ color: ACCENT_SOFT }}>{l.label}</span>
